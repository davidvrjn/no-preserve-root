...
public:
	...
	std::shared_ptr<InventoryComponent> clone() const override; // Will perform a deep copy of owned children.
	...
	// - if ownsChildren == true, the component will be stored as a shared_ptr and owned by this group.
	...
	//       1. Check component->getOwner(); if non-null, call previousOwner->remove(component) to detach it.
	//       2. Insert the component into this group's ownedComponents and call component->setOwner(shared_from_this()).
	//       3. Ensure invariants (component now has exactly one owner).
	//   This auto-move keeps ownership deterministic and simplifies moving plants between plots/storage.
