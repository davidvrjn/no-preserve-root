...
TEST_SUITE("Group Operations") {
    ...
    TEST_CASE("Mixed owning and non-owning groups") {
        ...
        // Prices
        CHECK(storage->getPrice() ==
              doctest::Approx(rose1->getPrice() + rose2->getPrice() + cactus->getPrice()));
        CHECK(summerView->getPrice() == doctest::Approx(rose1->getPrice() + rose2->getPrice()));
