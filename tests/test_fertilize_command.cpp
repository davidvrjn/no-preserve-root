...
TEST_CASE("FertilizeCommand - Serialization contains command type")
{
    auto nursery = std::make_shared<Nursery>();
    auto rose = std::make_shared<Rose>();
    auto cmd = std::make_unique<FertilizeCommand>(rose, nursery);
    ...
    CHECK(serialized.find("FertilizeCommand") != std::string::npos);
}

TEST_CASE("FertilizeCommand - Serialization contains status")
{
    auto nursery = std::make_shared<Nursery>();
    auto rose = std::make_shared<Rose>();
    auto cmd = std::make_unique<FertilizeCommand>(rose, nursery);
    ...
    CHECK(serialized.find("Completed") != std::string::npos);
}

TEST_CASE("FertilizeCommand - Serialization contains target ID")
{
    auto nursery = std::make_shared<Nursery>();
    auto rose = std::make_shared<Rose>();
    auto cmd = std::make_unique<FertilizeCommand>(rose, nursery);
    ...
    std::string serialized = cmd->serialize();
    std::string idStr = std::to_string(rose->getId());
    
    CHECK(serialized.find(idStr) != std::string::npos);
}

TEST_CASE("FertilizeCommand - Deserialization restores status")
{
    auto nursery = std::make_shared<Nursery>();
    auto cmd = std::make_unique<FertilizeCommand>(nullptr, nursery);
    
    std::string data = "FertilizeCommand|Completed|42";
    cmd->deserialize(data);
    
    CHECK(cmd->getStatus() == Command::Status::Completed);
}

TEST_CASE("FertilizeCommand - Deserialization restores target ID")
{
    auto nursery = std::make_shared<Nursery>();
    auto cmd = std::make_unique<FertilizeCommand>(nullptr, nursery);
    
    std::string data = "FertilizeCommand|Pending|12345";
    cmd->deserialize(data);
    
    CHECK(cmd->getTargetId() == 12345);
}

TEST_CASE("FertilizeCommand - Round-trip serialization")
{
    auto nursery = std::make_shared<Nursery>();
    auto rose = std::make_shared<Rose>();
    auto cmd1 = std::make_unique<FertilizeCommand>(rose, nursery);
    cmd1->setStatus(Command::Status::Failed);
    
    std::string serialized = cmd1->serialize();
    
    auto cmd2 = std::make_unique<FertilizeCommand>(nullptr, nursery);
    cmd2->deserialize(serialized);
    
    CHECK(cmd2->getStatus() == cmd1->getStatus());
    CHECK(cmd2->getTargetId() == cmd1->getTargetId());
}

TEST_CASE("FertilizeCommand - Deserialization handles all status types")
{
    auto nursery = std::make_shared<Nursery>();
    
    auto cmd1 = std::make_unique<FertilizeCommand>(nullptr, nursery);
    cmd1->deserialize("FertilizeCommand|Pending|1");
    CHECK(cmd1->getStatus() == Command::Status::Pending);
    
    auto cmd2 = std::make_unique<FertilizeCommand>(nullptr, nursery);
    cmd2->deserialize("FertilizeCommand|Completed|2");
    CHECK(cmd2->getStatus() == Command::Status::Completed);
    
    auto cmd3 = std::make_unique<FertilizeCommand>(nullptr, nursery);
    cmd3->deserialize("FertilizeCommand|Failed|3");
    CHECK(cmd3->getStatus() == Command::Status::Failed);
    
    auto cmd4 = std::make_unique<FertilizeCommand>(nullptr, nursery);
    cmd4->deserialize("FertilizeCommand|Cancelled|4");
    CHECK(cmd4->getStatus() == Command::Status::Cancelled);
}
