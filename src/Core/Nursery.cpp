...
Season Nursery::getCurrentSeason() const {
    ...
    if (dayInCycle < 30) return Season::SPRING;
    else if (dayInCycle < 60) return Season::SUMMER;
    else if (dayInCycle < 90) return Season::FALL;
    else return Season::WINTER;
}

void Nursery::addKnownPlantType(const std::string& plantType) {
    // Only add if not already in the list
    if (std::find(knownPlantTypes.begin(), knownPlantTypes.end(), plantType) == knownPlantTypes.end()) {
...
void Nursery::spawnCustomer() {
    ...
    if (isPurchase) {
        ...
        if (wantsDecorators) {
            ...
            for (int i = 0; i < numDecorators && i < static_cast<int>(availableDecorators.size()); ++i) {
